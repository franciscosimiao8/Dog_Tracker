...
def info_ga():
	...
	print '''
     %sCOMANDO                      DESCRICAO%s
  -------------       -------------------------------------

   get_data           quantidade de amigos
   get_info           informacoes
   dump_id            pega o id
   dump_phone         puxa o numero dos amigos
   dump_mail          puxa o email dos amigos
'''%(G,W)
...
'''
	...
	if 'dump_' in cek.lower() and cek.lower().split('_')[2] == 'id':
		...
		if cek == '':
			...
			print "[!] command '"+cek+"' not found"
			print '[!] aperte help para mostrar os comandos'
			main()
  except KeyboardInterrupt:
	main()
  except IndexError:
